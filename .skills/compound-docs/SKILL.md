...
preconditions:
  ...
  - Solution has been verified working via Frappe MCP, bench console inside docker, or browser tests
---
...
**Organization:** Single-file architecture - each problem documented as one markdown file in its symptom category directory (e.g., `docs/solutions/performance-issues/n-plus-one-price-logs.md`). Files use YAML frontmatter for metadata and searchability.
...
- Frappe version
- ERPNext version
- Phase (1-3 or post-implementation)
- OS version
- File/line references

**BLOCKING REQUIREMENT:** If critical context is missing (module name, exact error, phase, or resolution steps), ask user and WAIT for response before proceeding to Step 3:
...
3. What phase were you in? (1-3 or post-implementation)
...
- `missing-select-fields-pricing-intelligence-20260322.md`
- `cache-invalidation-react-query-20260322.md`
- `hook-ordering-doc-events-20260322.md`
</step>
...
- Affects multiple modules OR foundational phase (Phase 1)
- Non-obvious solution
...
- Foundational requirement (Frappe API, hooks, background jobs, etc.)
...
2. **Gather context:**
   ...
   - Symptom: Snapshot job taking >30 seconds, N+1 query when loading sales lines
   - Failed attempts: Added pagination (didn't help), checked RQ job performance
   - Solution: Used `frappe.get_list` with explicit `fields` and `pluck` instead of `frappe.get_doc` in loop
   - Root cause: Missing select_fields causing full document load per iteration
3. **Check existing:** No similar issue found
4. **Generate filename:** `n-plus-one-snapshot-job-pricing-intelligence-20260322.md`
5. **Validate YAML:**
   ```yaml
   module: Pricing Intelligence
   date: 2026-03-22
   ...
   symptoms:
     - "N+1 query when loading sales invoice items for regime"
     - "Snapshot job taking >30 seconds"
   root_cause: missing_select_fields
   frappe_version: 16.0.0
   resolution_type: code_fix
   severity: high
   tags: [n-plus-one, frappe-get-list, performance]
   ```
   Valid
6. **Create documentation:**
   - `docs/solutions/performance-issues/n-plus-one-snapshot-job-pricing-intelligence-20260322.md`

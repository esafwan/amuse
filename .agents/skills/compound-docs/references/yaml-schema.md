...
- **module** (string): Module name (e.g., "Pricing Intelligence") or "System" for system-wide issues
...
- **component** (enum): One of [frappe_doctype, frappe_controller, frappe_api, service_layer, background_job, database, frontend_react, frontend_desk, hooks, pricing_engine, customer_management, billing, reporting, development_workflow, testing_framework, documentation, tooling]
- **symptoms** (array): 1-5 specific observable symptoms
- **root_cause** (enum): One of [missing_link, missing_select_fields, missing_index, wrong_api, scope_issue, hook_ordering, async_timing, memory_leak, config_error, logic_error, test_isolation, missing_validation, missing_permission, missing_workflow_step, inadequate_documentation, missing_tooling, incomplete_setup, cache_issue, frontend_state]
- **resolution_type** (enum): One of [code_fix, patch, config_change, test_fix, dependency_update, environment_setup, workflow_improvement, documentation_update, tooling_addition, fixture_update, custom_field_fix]
- **severity** (enum): One of [critical, high, medium, low]

## Optional Fields

- **frappe_version** (string): Frappe version in X.Y.Z format
- **erpnext_version** (string): ERPNext version in X.Y.Z format
...
5. frappe_version (if provided) must match X.Y.Z format
...
symptoms:
  - "N+1 query when loading price change logs"

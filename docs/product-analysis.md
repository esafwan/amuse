...
|---|---|
| **POS** | Session management, item catalog, cart, checkout, payment entry creation |
| **Billing** | Sales Invoice list, detail view, submit, cancel |
| **Customers** | List, search by name/phone/email, group filter, detail view, create/update |
...
`frontend/src/lib/print.ts` exists but the POS checkout modal has no "Print Receipt" action after a successful payment.
...
5. **Print receipt from POS** — low effort, high operator value; `print.ts` is already scaffolded.

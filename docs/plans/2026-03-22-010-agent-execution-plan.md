...
> Method: Compound Engineering (see `.claude/skills/compound-docs/`)
> Reference: HUF permissions architecture (`tridz-dev/huf`, branch `feature/huf-permissions-design`)
...
Wrap routes with `<PermissionsProvider>` inside `<QueryClientProvider>`.
